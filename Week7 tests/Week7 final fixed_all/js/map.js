...
function createChoroMap(lat,lon,zl){
	map = L.map('choromap').setView([chorolat,chorolon], chorozl);
...
function getChoroGeoJSON(){
	...
	$.getJSON(chorogeojsonPath,function(data){
		...
		mapChoroGeoJSON.forEach('Mismanaged_plastic_waste_2010_tonnes','Coverage','annual_deaths_rate_per100,000')
	})
}
